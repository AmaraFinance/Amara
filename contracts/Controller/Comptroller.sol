...
contract Comptroller is ComptrollerV4Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when MARA is granted by admin
    event CompGranted(address recipient, uint amount);

    /// @notice The threshold above which the flywheel transfers MARA, in wei
    uint public constant compClaimThreshold = 0.001e18;

    /// @notice The initial MARA index for a market
    ...
    function mintAllowed(address aToken, address minter, uint mintAmount) external returns (uint) {
        ...
        distributeSupplierComp(aToken, minter, false);
    ...
    function redeemAllowed(address aToken, address redeemer, uint redeemTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateCompSupplyIndex(aToken);
        distributeSupplierComp(aToken, redeemer, false);
    ...
    function borrowAllowed(address aToken, address borrower, uint borrowAmount) external returns (uint) {
        ...
        distributeBorrowerComp(aToken, borrower, borrowIndex, false);
    ...
    function seizeAllowed(
        ...
        // Keep the flywheel moving
        updateCompSupplyIndex(aTokenCollateral);
        distributeSupplierComp(aTokenCollateral, borrower, false);
        distributeSupplierComp(aTokenCollateral, liquidator, false);
    ...
    function transferAllowed(address aToken, address src, address dst, uint transferTokens) external returns (uint) {
        ...
        // Keep the flywheel moving
        updateCompSupplyIndex(aToken);
        distributeSupplierComp(aToken, src, false);
        distributeSupplierComp(aToken, dst, false);
    ...
    function liquidateCalculateSeizeTokens(address aTokenBorrowed, address aTokenCollateral, uint actualRepayAmount) external view returns (uint, uint) {
         ...
         */
        uint exchangeRateMantissa = AToken(aTokenCollateral).exchangeRateStored();
    ...
    function _supportMarket(AToken aToken) external returns (uint) {
        ...
        aToken.isAToken();
        // Sanity check to make sure its really a AToken
    ...
    /**
     * @notice Recalculate and update MARA speeds for all MARA markets
     */
    ...
    function refreshCompSpeedsInternal() internal {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            compSpeeds[address(aToken)] = newSpeed;
            emit CompSpeedUpdated(aToken, newSpeed);
        }
    }

    /**
     * @notice Accrue MARA to the market by updating the supply index
    ...
    /**
     * @notice Accrue MARA to the market by updating the borrow index
    ...
    /**
     * @notice Calculate MARA accrued by a supplier and possibly transfer it to them
     * @param aToken The market in which the supplier is interacting
     * @param supplier The address of the supplier to distribute MARA to
     */
    function distributeSupplierComp(address aToken, address supplier, bool distributeAll) internal {
        CompMarketState storage supplyState = compSupplyState[aToken];
    ...
    /**
     * @notice Calculate MARA accrued by a borrower and possibly transfer it to them
     * @dev Borrowers will not begin to accrue until after the first interaction with the protocol.
     * @param aToken The market in which the borrower is interacting
     * @param borrower The address of the borrower to distribute MARA to
     */
    function distributeBorrowerComp(address aToken, address borrower, Exp memory marketBorrowIndex, bool distributeAll) internal {
        CompMarketState storage borrowState = compBorrowState[aToken];
    ...
    /**
     * @notice Transfer AMA to the user, if they are above the threshold
     * @dev Note: If there is not enough MARA, we do not perform the transfer all.
     * @param user The address of the user to transfer AMA to
     * @param userAccrued The amount of MARA to (possibly) transfer
     * @return The amount of AMA which was NOT transferred to the user
     */
    function transferComp(address user, uint userAccrued, uint threshold) internal returns (uint) {
        if (userAccrued >= threshold && userAccrued > 0) {
            Mara mara = Mara(getCompAddress());
            uint compRemaining = mara.balanceOf(address(this));
            if (userAccrued <= compRemaining) {
                mara.transfer(user, userAccrued);
                return 0;
            }
        }
        return userAccrued;
    ...
    /**
     * @notice Claim all the MARA accrued by holder in all markets
     * @param holder The address to claim MARA for
     */
    ...
    /**
     * @notice Claim all the MARA accrued by holder in the specified markets
     * @param holder The address to claim MARA for
     * @param aTokens The list of markets to claim MARA in
     */
    function claimComp(address holder, AToken[] memory aTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimComp(holders, aTokens, true, true);
    }

    /**
     * @notice Claim all MARA accrued by the holders
     * @param holders The addresses to claim MARA for
     * @param aTokens The list of markets to claim MARA in
     * @param borrowers Whether or not to claim MARA earned by borrowing
     * @param suppliers Whether or not to claim MARA earned by supplying
     */
    function claimComp(address[] memory holders, AToken[] memory aTokens, bool borrowers, bool suppliers) public {
        for (uint i = 0; i < aTokens.length; i++) {
            ...
            if (borrowers == true) {
                ...
                for (uint j = 0; j < holders.length; j++) {
                    distributeBorrowerComp(address(aToken), holders[j], borrowIndex, true);
                }
            }
            if (suppliers == true) {
                updateCompSupplyIndex(address(aToken));
                for (uint j = 0; j < holders.length; j++) {
                    distributeSupplierComp(address(aToken), holders[j], true);
                }
            }
        }
    }
    ...
    /**
     * @notice Transfer MARA to the user
     * @dev Note: If there is not enough MARA, we do not perform the transfer all.
     * @param user The address of the user to transfer MARA to
     * @param amount The amount of MARA to (possibly) transfer
     * @return The amount of MARA which was NOT transferred to the user
     */
    function grantCompInternal(address user, uint amount) internal returns (uint) {
        Mara mara = Mara(getCompAddress());
        uint compRemaining = mara.balanceOf(address(this));
        if (amount <= compRemaining) {
            mara.transfer(user, amount);
            return 0;
        }
        return amount;
    }

    /*** MARA Distribution Admin ***/

    /**
     * @notice Transfer MARA to the recipient
     * @dev Note: If there is not enough MARA, we do not perform the transfer all.
     * @param recipient The address of the recipient to transfer MARA to
     * @param amount The amount of MARA to (possibly) transfer
     */
    function _grantComp(address recipient, uint amount) public {
        require(adminOrInitializing(), "only admin can grant MARA");
    ...
    /**
     * @notice Set the amount of MARA distributed per block
     * @param compRate_ The amount of MARA wei per block to distribute
     */
    function _setCompRate(uint compRate_) public {
        require(adminOrInitializing(), "only admin can change MARA rate");
    ...
    /**
     * @notice Add markets to compMarkets, allowing them to earn MARA in the flywheel
     * @param aTokens The addresses of the markets to add
     */
    function _addCompMarkets(address[] memory aTokens) public {
        require(adminOrInitializing(), "only admin can add MARA market");

        for (uint i = 0; i < aTokens.length; i++) {
            _addCompMarketInternal(aTokens[i]);
        }

        refreshCompSpeedsInternal();
    }

    function _addCompMarketInternal(address aToken) internal {
        Market storage market = markets[aToken];
        require(market.isListed == true, "MARA market is not listed");
        require(market.isComped == false, "MARA market already added");

        market.isComped = true;
        emit MarketComped(AToken(aToken), true);

        if (compSupplyState[aToken].index == 0 && compSupplyState[aToken].block == 0) {
            compSupplyState[aToken] = CompMarketState({
        ...
        if (compBorrowState[aToken].index == 0 && compBorrowState[aToken].block == 0) {
            compBorrowState[aToken] = CompMarketState({
            index : compInitialIndex,
            block : safe32(getBlockNumber(), "block number exceeds 32 bits")
            });
    ...
    /**
     * @notice Remove a market from compMarkets, preventing it from earning MARA in the flywheel
     * @param aToken The address of the market to drop
     */
    function _dropCompMarket(address aToken) public {
        require(msg.sender == admin, "only admin can drop MARA market");

        Market storage market = markets[aToken];
        require(market.isComped == true, "market is not a MARA market");
